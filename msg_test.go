...
func TestEncodeMessage(t *testing.T) {
	...
	f := func(msg testMsg) bool {
		bytes, err := p.EncodeMessage([ChannelIDSize]byte{}, msg)
		assert.Nil(t, err)
		assert.Equal(t, append(append(make([]byte, ChannelIDSize), byte(o)), msg.Write()...), bytes)
...
func TestDecodeMessage(t *testing.T) {
	...
	f := func(msg testMsg) bool {
		_, resultO, resultM, err := p.DecodeMessage(append(append(make([]byte, ChannelIDSize), byte(o)), msg.Write()...))
		assert.Nil(t, err)
...
func TestEncodeMessageError(t *testing.T) {
	...
	_, err = p.EncodeMessage([ChannelIDSize]byte{}, testMsg{Text: "hello"})
	assert.Error(t, err)
...
func TestDecodeMessageError(t *testing.T) {
	...
	_, resultO, resultM, err = p.DecodeMessage(append(append(make([]byte, ChannelIDSize), byte(o)), testMessage.Write()...))
	assert.Equal(t, o, resultO)
	...
	_, resultO, resultM, err = p.DecodeMessage(append(append(make([]byte, ChannelIDSize), byte(222)), testMessage.Write()...))
	assert.Equal(t, Opcode(byte(222)), resultO)
	assert.Nil(t, resultM)
	assert.Error(t, err)

	// test decode nil
	_, resultO, resultM, err = p.DecodeMessage(nil)
	assert.Equal(t, OpcodeNil, resultO)
	assert.Equal(t, nil, resultM)
	assert.Error(t, err)

	// test decode with nil content
	_, resultO, resultM, err = p.DecodeMessage(append(append(make([]byte, ChannelIDSize), byte(o))))
	assert.Equal(t, o, resultO)
	assert.Nil(t, resultM)
	assert.Error(t, err)
	...
	_, resultO, resultM, err = p.DecodeMessage(append([]byte{byte(o)}, testMessage.Write()...))
	assert.Equal(t, OpcodeNil, resultO)
	assert.Equal(t, nil, resultM)
	assert.Error(t, err)
	//assert.Equal(t, headerError, errors.Cause(err))
